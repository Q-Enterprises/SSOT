...
State: (\mathbf{s}{i,f}=[x{i,f},y_{i,f},\theta_{i,f},\mathrm{scale}{i,f},\mathrm{tone}{i,f}]).
...
Camera per frame f:
(\mathbf{c}_{f}=[\mathrm{pan}_f,\mathrm{tilt}_f,\mathrm{zoom}f]=\mathrm{spline}(f;{\mathbf{c}{t_k}})).
...
(\mathbf{s}{i,f}^{\ast}=\sum{k\in{\text{Queen Boo},\text{CiCi},\text{Sol.F1}}}\alpha_{k,f},\mathbf{W}k,\mathbf{s}{i,f}), where (\alpha_{k,f}) are policy weights (quorum, tone), (\mathbf{W}_k) encode stabilization and discipline.
